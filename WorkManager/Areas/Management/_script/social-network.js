var pageIndex = 1;
var URLC = "/Management/SocialNetwork/Action";
var URLA = "/Management/SocialNetwork";
var SocialNetworkController = {
    init: function () {
        SocialNetworkController.registerEvent();
    },
    registerEvent: function () {
        ...
        $("#btnSearch").off("click").on("click", function () {
            SocialNetworkController.DataList(1);
        });
        $("#btnUpdate").off("click").on("click", function () {
            ...
            // submit
            if (flg) {
                SocialNetworkController.Update();
            }
            else {
                Notifization.Error(MessageText.Datamissing);
            }
                    ...
                    if (result.status === 200) {
                        ...
                        $.each(result.data, function (index, item) {
                            ...
                            if (id.length > 0)
                                id = id.trim();
                            //  role
                            var action = HelperModel.RolePermission(result.role, "SocialNetworkController", id);
                            //
                        ...
                        if (parseInt(totalPage) > 1) {
                            Paging.Pagination("#Pagination", totalPage, currentPage, SocialNetworkController.DataList);
                        }
                        return;
                    }
                    else {
        ...
        AjaxFrom.POST({
            ...
            success: function (response) {
                if (response !== null) {
                    if (response.status === 200) {
                        Notifization.Success(response.message);
                        SocialNetworkController.DataList(pageIndex);
                        return;
                    }
                    else {
                        Notifization.Error(response.message);
    ...
    ConfirmDelete: function (id) {
        Confirm.Delete(id, SocialNetworkController.Delete, null, null);
    }
};

SocialNetworkController.init();
