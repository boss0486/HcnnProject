var pageIndex = 1;
var URLC = "/Management/AttachmentCategory/Action";
var URLA = "/Management/AttachmentCategory";
var attachmentCategoryController = {
    init: function () {
        attachmentCategoryController.registerEvent();
    },
    registerEvent: function () {
        $('#btnCreate').off('click').on('click', function () {
            ...
            if (flg) {
                attachmentCategoryController.Create();
            }
            else {
        ...
        $('#btnSearch').off('click').on('click', function () {
            attachmentCategoryController.DataList(1);
        });
        $('#btnUpdate').off('click').on('click', function () {
            ...
            // submit
            if (flg) {
                attachmentCategoryController.Update();
            }
            else {
                Notifization.Error(MessageText.Datamissing);
            }
                ...
                if (result !== null) {
                    if (result.status === 200) {
                        ...
                        $.each(result.data, function (index, item) {
                            ...
                            //
                            //  role 
                            var action = HelperModel.RolePermission(result.role, "attachmentCategoryController", id);
                            //
                            ...
                            var subMenu = item.SubOptionData;
                            if (subMenu !== undefined && subMenu !== null && subMenu.length > 0) {
                                rowData += attachmentCategoryController.GetDataList(index, subMenu, _level, result.role);
                            }
                        });
                        $('tbody#TblData').html(rowData);
                        if (parseInt(totalPage) > 1) {
                            Paging.Pagination("#Pagination", totalPage, currentPage, attachmentCategoryController.DataList);
                        }
                        return;
                    }
                    else {
    ...
    GetDataList: function (_index, lstModel, _level, _role) {
        var rowData = '';
        //
        if (lstModel.length > 0) {
            ...
            $.each(lstModel, function (index, item) {
                ...
                var role = _role;
                //  role
                var action = HelperModel.RolePermission(role, "attachmentCategoryController", id);
                //
                // icon sort
                var _orderId = item.OrderID;
                var _actionSort = `<i data-sortup='btn-sort-up' data-id='${id}' data-order'${_orderId}' class='fas fa-arrow-circle-up icon-mnsort'></i> <i data-sortdown ='btn-sort-down' data-id='${id}' data-order'${_orderId}' class='fas fa-arrow-circle-down icon-mnsort'></i>`;
                ...
                if (subMenu !== undefined && subMenu !== null && subMenu.length > 0) {
                    rowData += attachmentCategoryController.GetDataList(_index, subMenu, _level, _role);
                }
            });
        }
        return rowData;
        ...
        AjaxFrom.POST({
            ...
            success: function (response) {
                if (response !== null) {
                    if (response.status === 200) {
                        Notifization.Success(response.message);
                        attachmentCategoryController.DataList(pageIndex);
                        return;
                    }
                    else {
                        Notifization.Error(response.message);
    ...
    ConfirmDelete: function (id) {
        Confirm.Delete(id, attachmentCategoryController.Delete, null, null);
    }
};

attachmentCategoryController.init();
