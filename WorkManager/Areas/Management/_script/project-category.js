var pageIndex = 1;
var URLC = "/Management/projectCategory/Action";
var URLA = "/Management/projectCategory";
var projectCategoryController = {
    init: function () {
        projectCategoryController.registerEvent();
    },
    registerEvent: function () {
        ...
        $('#btnSearch').off('click').on('click', function () {
            projectCategoryController.DataList(1);
        });
            ...
            success: function (response) {
                if (response != null) {
                    if (response.status == 200) {
                        ...
                        FData.ResetForm();
                        projectCategoryController.GetCategory();
                        return;
                    }
                    else {
                    ...
                    if (result.status == 200) {
                        ...
                        $.each(result.data, function (index, item) {
                            ...
                            if (id.length > 0)
                                id = id.trim();
                            //  role
                            var action = HelperModel.RolePermission(result.role, "projectCategoryController", id);
                            // icon sort
                            ...
                            if (subMenu != undefined && subMenu != null && subMenu.length > 0) {
                                rowData += projectCategoryController.SubData(index, subMenu, 0, result.role);
                            }
                        });
                        $('tbody#TblData').html(rowData);
                        if (parseInt(totalPage) > 1) {
                            Paging.Pagination("#Pagination", totalPage, currentPage, projectCategoryController.DataList);
                        }
                        return;
                    }
                }
    ...
    SubData: function (_index, lstModel, _level, _role) {
        ...
        if (lstModel.length > 0) {
            ...
            $.each(lstModel, function (index, item) {
                ...
                var _actionSort = `<i data-sortup='btn-sort-up' data-id='${id}' data-order'${_orderId}' class='fas fa-arrow-circle-up icon-mnsort'></i> <i data-sortdown ='btn-sort-down' data-id='${id}' data-order'${_orderId}' class='fas fa-arrow-circle-down icon-mnsort'></i>`;
                //
                var action = HelperModel.RolePermission(_role, "projectCategoryController", id);
                var _title = SubStringText.SubTitle(item.Title);
                var rowNum = ''; //parseInt(index) + (parseInt(currentPage) - 1) * parseInt(pageSize);
                _index = _level * 15;
                rowData += `
                ...
                var subMenu = item.SubData;
                if (subMenu != undefined && subMenu != null && subMenu.length > 0) {
                    rowData += projectCategoryController.SubData(_index, subMenu, _level, _role);
                }
            });
        }
        return rowData;
        ...
        AjaxFrom.POST({
            ...
            success: function (response) {
                if (response != null) {
                    if (response.status == 200) {
                        Notifization.Success(response.message);
                        projectCategoryController.DataList(pageIndex);
                        return;
                    }
                    else {
                        Notifization.Error(response.message);
    ...
    ConfirmDelete: function (id) {
        Confirm.Delete(id, projectCategoryController.Delete, null, null);
    },
                ...
                if (result != null) {
                    if (result.status == 200) {
                        ...
                        $.each(result.data, function (index, item) {
                            ...
                            if (subMenu != undefined && subMenu != null && subMenu.length > 0) {
                                rowData += projectCategoryController.GetSubCategory(index, subMenu, _level, _id);
                            }
                            rowData += '</li>';
    ...
    GetSubCategory: function (_index, lstModel, _level, _id) {
        ...
        if (lstModel.length > 0) {
            ...
            $.each(lstModel, function (index, item) {
                ...
                if (subMenu != undefined && subMenu != null && subMenu.length > 0) {
                    rowData += projectCategoryController.GetSubCategory(_index, subMenu, _level, _id);
                }
                rowData += '</li>';
            });
            rowData += `</ul>`;
