var pageIndex = 1;
var URLC = "/Management/Department/Action";
var URLA = "/Management/Department";
var departmentController = {
    ...
    registerEvent: function () {
        $("#btnCreate").off("click").on("click", function () {
            ...
            if (txtTitle === "") {
                $("#lblTitle").html("Không được để trống phòng ban");
                flg = false;
            }
            else if (txtTitle.length < 1 || txtTitle.length > 80) {
                $("#lblTitle").html("Phòng ban giới hạn [1-80] ký tự");
                flg = false;
            }
            else if (!FormatKeyword.test(txtTitle)) {
                $("#lblTitle").html("Phòng ban không hợp lệ");
                flg = false;
            }
            else {
                $("#lblTitle").html("");
        ...
        $("#btnSearch").off("click").on("click", function () {
            departmentController.DataList(1);
        });
                    ...
                    if (result.status === 200) {
                        ...
                        $.each(result.data, function (index, item) {
                            ...
                            if (id.length > 0)
                                id = id.trim();
                            //  role
                            var action = HelperModel.RolePermission(result.role, "departmentController", id);
                            //
                        ...
                        if (parseInt(totalPage) > 1) {
                            Paging.Pagination("#Pagination", totalPage, currentPage, departmentController.DataList);
                        }
                        return;
                    }
                    else {
        ...
        AjaxFrom.POST({
            ...
            success: function (response) {
                if (response !== null) {
                    if (response.status === 200) {
                        Notifization.Success(response.message);
                        departmentController.DataList(pageIndex);
                        return;
                    }
                    else {
                        Notifization.Error(response.message);
    ...
    ConfirmDelete: function (id) {
        Confirm.Delete(id, departmentController.Delete, null, null);
    }
};
