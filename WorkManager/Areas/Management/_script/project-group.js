var pageIndex = 1;
var URLC = "/Management/ProjectGroup/Action";
var URLA = "/Management/ProjectGroup";
var projectGroupController = {
    init: function () {
        projectGroupController.registerEvent();
    },
    registerEvent: function () {
        ...
        $('#btnSearch').off('click').on('click', function () {
            projectGroupController.DataList(1);
        });
                    ...
                    if (result.status === 200) {
                        ...
                        $.each(result.data, function (index, item) {
                            ...
                            if (id.length > 0)
                                id = id.trim();
                            //  role
                            var action = HelperModel.RolePermission(result.role, "projectGroupController", id);
                            //
                        ...
                        if (parseInt(totalPage) > 1) {
                            Paging.Pagination("#Pagination", totalPage, currentPage, projectGroupController.DataList);
                        }
                        return;
                    }
                    else {
        ...
        AjaxFrom.POST({
            ...
            success: function (response) {
                if (response !== null) {
                    if (response.status === 200) {
                        Notifization.Success(response.message);
                        projectGroupController.DataList(pageIndex);
                        return;
                    }
                    else {
                        Notifization.Error(response.message);
    ...
    ConfirmDelete: function (id) {
        Confirm.Delete(id, projectGroupController.Delete, null, null);
    }
};

projectGroupController.init();
