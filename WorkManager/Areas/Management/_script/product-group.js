var pageIndex = 1;
var URLC = "/Management/ProductGroup/Action";
var URLA = "/Management/ProductGroup";
var productGroupController = {
    init: function () {
        productGroupController.registerEvent();
    },
    registerEvent: function () {
        ...
        $('#btnSearch').off('click').on('click', function () {
            productGroupController.DataList(1);
        });
                    ...
                    if (result.status === 200) {
                        ...
                        $.each(result.data, function (index, item) {
                            ...
                            if (id.length > 0)
                                id = id.trim();
                            //  role
                            var action = HelperModel.RolePermission(result.role, "productGroupController", id);
                            //
                        ...
                        if (parseInt(totalPage) > 1) {
                            Paging.Pagination("#Pagination", totalPage, currentPage, productGroupController.DataList);
                        }
                        return;
                    }
                    else {
        ...
        AjaxFrom.POST({
            ...
            success: function (response) {
                if (response !== null) {
                    if (response.status === 200) {
                        Notifization.Success(response.message);
                        productGroupController.DataList(pageIndex);
                        return;
                    }
                    else {
                        Notifization.Error(response.message);
    ...
    ConfirmDelete: function (id) {
        Confirm.Delete(id, productGroupController.Delete, null, null);
    }
};

productGroupController.init();
