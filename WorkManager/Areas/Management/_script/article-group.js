var pageIndex = 1;
var URLC = "/Management/ArticleGroup/Action";
var URLA = "/Management/ArticleGroup";
var articleGroupController = {
    init: function () {
        articleGroupController.registerEvent();
    },
    registerEvent: function () {
        ...
        $('#btnSearch').off('click').on('click', function () {
            articleGroupController.DataList(1);
        });
                    ...
                    if (result.status === 200) {
                        ...
                        $.each(result.data, function (index, item) {
                            ...
                            if (id.length > 0)
                                id = id.trim();
                            //  role
                            var action = HelperModel.RolePermission(result.role, "articleGroupController", id);
                            //
                        ...
                        if (parseInt(totalPage) > 1) {
                            Paging.Pagination("#Pagination", totalPage, currentPage, articleGroupController.DataList);
                        }
                        return;
                    }
                    else {
        ...
        AjaxFrom.POST({
            ...
            success: function (response) {
                if (response !== null) {
                    if (response.status === 200) {
                        Notifization.Success(response.message);
                        articleGroupController.DataList(pageIndex);
                        return;
                    }
                    else {
                        Notifization.Error(response.message);
    ...
    ConfirmDelete: function (id) {
        Confirm.Delete(id, articleGroupController.Delete, null, null);
    }
};

articleGroupController.init();
